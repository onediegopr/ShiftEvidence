...
## AI-REPORT-1B Follow-up

Date: 2026-05-27.

AI-REPORT-1B added strict success mode:
...
- output folder: `qa-artifacts/ai-report-1b/`;
- PDF generated by real renderer: YES;
- `providerStatus`: `unavailable`;
- `GEMINI_API_KEY` in local environment: missing;
- status: PARCIAL.

The script now fails the run if `providerStatus` is not `success`, preventing false Gemini success reporting.
