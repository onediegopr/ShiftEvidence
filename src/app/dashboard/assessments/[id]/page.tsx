...
function buildMatrixHref(params: {
  ...
  url.searchParams.set("tab", params.tab ?? "report");
...
export default async function AssessmentDetailPage({
  ...
  const hasReport = assessmentScore !== undefined && assessmentScore !== null;
    ...
    <main className="dashboard-page assessment-detail-page">
      <section className="dashboard-hero glass-card">
        <div>
          ...
          <p>
            {assessment.clientLabel ?? "No client label yet"} &bull; {assessment.assessmentType} &bull;{" "}
      ...
      <nav className="tabs-container" aria-label="Assessment tabs" style={{ marginTop: "1rem" }}>
        <Link
          href={`/dashboard/assessments/${assessment.id}?tab=basics`}
          className={`tab-btn ${activeTab === "basics" ? "active" : ""}`}
        >
          <span className="tab-progress-dot completed"></span>
          Intake & Basics
        </Link>
        <Link
        ...
        >
          <span 
            ...
            style={{ background: isUploadUnlocked ? (evidenceFiles.length > 0 ? "#10b981" : "#f59e0b") : "#ef4444" }}
          ></span>
          Evidence Upload
        ...
        >
          <span 
            className="tab-progress-dot" 
            style={{ background: hasInventory ? "#10b981" : (evidenceFiles.length > 0 ? "#f59e0b" : "#475569") }}
          ></span>
          Parsed Inventory
        </Link>
        <Link
          href={`/dashboard/assessments/${assessment.id}?tab=report`}
      ...
      {/* Global summary stats card panel (Always visible) */}
      ...
      {activeTab === "evidence" && (
        <section id="evidence-upload" className="assessment-section glass-card">
          ...
          <div className="assessment-status-row">
            ...
            {renderStatusPill(
              isUploadUnlocked ? "Upload gate: ready" : "Upload gate: blocked",
              isUploadUnlocked ? "good" : "warning",
            )}
          </div>

          <div className="assessment-evidence-grid">
            <div className="assessment-evidence-form">
              {!isUploadUnlocked ? (
                <div className="locked-overlay-card glass-card" style={{ padding: 0 }}>
                  <div className="locked-overlay-content">
                    <div className="locked-overlay-badge">
                      <Lock size={16} />
                      Upload Locked
                    </div>
                    <strong style={{ color: "white", fontSize: "1.15rem" }}>
                      Basics and manual intake context required first.
                    </strong>
                    <p className="text-muted" style={{ fontSize: "0.9rem", margin: 0, lineHeight: "1.6" }}>
                      To ensure the parser interprets your RVTools file correctly and generates a reliable compatibility scorecard, you must first complete the baseline settings in the **Intake & Basics** tab.
                    </p>
                    ...
                    <div style={{ marginTop: "0.25rem" }}>
                      ...
                      <ul className="assessment-bullet-list" style={{ margin: 0, paddingLeft: "1.25rem" }}>
                        {uploadPrerequisites.missingPrerequisites.map((item) => (
                          <li key={item.key} style={{ fontSize: "0.88rem" }}>
                            {item.href ? (
                              <Link href={`/dashboard/assessments/${assessment.id}?tab=basics${item.href}`} style={{ color: "#38bdf8", textDecoration: "underline" }}>
                                {item.label}
                              </Link>
                            ) : (
                    ...
                    <Link href={`/dashboard/assessments/${assessment.id}?tab=basics`} className="btn btn-secondary btn-sm" style={{ width: "fit-content", marginTop: "0.5rem", border: "1px solid rgba(255, 255, 255, 0.15)" }}>
                      Go to Intake & Basics
                      <ArrowRight size={14} />
                    </Link>
                  </div>
                ...
                <form action={uploadEvidenceAction.bind(null, assessment.id)} encType="multipart/form-data" className="glass-card assessment-subcard">
                        ...
                        {!deleted ? (
                          <div className="assessment-evidence-actions" style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
                              ...
                              file.processingStatus === "processing" ? (
                                ...
                                <form action={parseRvtoolsEvidenceAction.bind(null, assessment.id, file.id)}>
                                  <button type="submit" className="btn btn-primary btn-glow btn-sm" style={{ padding: "0.45rem 1rem", fontSize: "0.85rem", background: "linear-gradient(135deg, #06b6d4 30%, #0891b2 100%)", border: "none" }}>
                                    ...
                                    {file.processingStatus === "parsed" ? "Reparse" : "Parse"}
                                  </button>
                                </form>
                              )
                            )}
                            ...
                            <form action={deleteEvidenceAction.bind(null, assessment.id, file.id)}>
                              <button type="submit" className="btn btn-secondary btn-sm assessment-danger-button" style={{ padding: "0.45rem 1rem", fontSize: "0.85rem" }}>
                                <Trash2 size={14} />
                                Delete
                              </button>
                            </form>
            ...
            <div className="assessment-inventory-copy glass-card assessment-subcard">
              {latestRvtoolsEvidence ? (
                ...
                <>
                  ...
                  <Link href={`/dashboard/assessments/${assessment.id}?tab=evidence`} className="btn btn-secondary btn-sm" style={{ width: "fit-content", marginTop: "0.5rem" }}>
            ...
            <div className="assessment-inventory-tables">
              <InventoryTable
                ...
                columns={[
                  ...
                  { key: "state", label: "Power", render: (row) => row.powerState ?? "—" },
                  { key: "cpu", label: "CPU", render: (row) => row.cpuCount ?? "—" },
                  { key: "memory", label: "Memory MB", render: (row) => row.memoryMb ?? "—" },
                  ...
                  { key: "ds", label: "Datastore", render: (row) => row.datastoreName ?? "—" },
                  { key: "host", label: "Host", render: (row) => row.hostName ?? "—" },
                  { key: "risk", label: "Risk", render: (row) => row.riskLevel ?? "—" },
                ]}
              />
              ...
              <InventoryTable
                ...
                columns={[
                  { key: "host", label: "Host", render: (row) => row.hostName },
                  { key: "cluster", label: "Cluster", render: (row) => row.clusterName ?? "—" },
                  { key: "model", label: "CPU model", render: (row) => row.cpuModel ?? "—" },
                  { key: "sockets", label: "Sockets", render: (row) => row.cpuSockets ?? "—" },
                  { key: "cores", label: "Cores", render: (row) => row.cpuCores ?? "—" },
              ...
              <InventoryTable
                ...
                columns={[
                  ...
                  { key: "type", label: "Type", render: (row) => row.datastoreType ?? "—" },
              ...
              <InventoryTable
                ...
                columns={[
                  { key: "vm", label: "VM", render: (row) => row.vmName ?? "—" },
                  { key: "name", label: "Snapshot", render: (row) => row.snapshotName ?? "—" },
                  { key: "created", label: "Created", render: (row) => dateLabel(row.createdAtSource) },
                  { key: "age", label: "Age days", render: (row) => row.ageDays ?? "—" },
      ...
      {activeTab === "report" && (
        <>
          ...
          <section id="risk-overview" className="assessment-section glass-card">
            ...
            <div className="assessment-preview-grid assessment-risk-grid">
              <article className="assessment-preview-card">
                ...
                <strong>{assessmentScore ? formatNumber(assessmentScore.readinessScore) : "—"}</strong>
              </article>
              <article className="assessment-preview-card">
                <span className="assessment-preview-label">Confidence score</span>
                <strong>{assessmentScore ? formatNumber(assessmentScore.confidenceScore) : "—"}</strong>
              </article>
              <article className="assessment-preview-card">
                <span className="assessment-preview-label">Risk level</span>
                <strong>{assessmentScore ? riskSeverityLabel(assessmentScore.riskLevel ?? "info") : "—"}</strong>
              </article>
              <article className="assessment-preview-card">
          ...
          <section className="assessment-section glass-card">
            ...
            <div className="assessment-status-row">
              {renderStatusPill(`Risk filter: ${filterRisk === "all" ? "All" : filterRisk}`, "neutral")}
              {renderStatusPill(
                `Power filter: ${filterPower === "all" ? "All" : filterPower === "powered_on" ? "Powered on" : "Powered off"}`,
                "neutral",
              )}
            ...
            <div className="assessment-filter-row">
              ...
              {(["all", "info", "low", "medium", "high", "critical"] as const).map((value) => (
                <Link
                  ...
                  className={`assessment-filter-chip${filterRisk === value ? " is-active" : ""}`}
                >
            ...
            <div className="assessment-filter-row">
              ...
              {([
                ...
                <Link
                  ...
                  className={`assessment-filter-chip${filterPower === value.key ? " is-active" : ""}`}
                >
                  {value.label}
                </Link>
              ))}
