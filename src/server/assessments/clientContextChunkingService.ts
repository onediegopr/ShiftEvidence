import {
  countClientContextCharacters,
  countClientContextWords,
} from "./clientContextValidation";

export type ClientContextChunk = {
  index: number;
  text: string;
  wordCount: number;
...
export type ClientContextChunkingOptions = {
...
export function countWords(text: string) {
  return countClientContextWords(text);
}

export function countCharacters(text: string) {
  return countClientContextCharacters(text);
}
...
export function chunkClientContextText(
  text: string,
  options: ClientContextChunkingOptions = {},
): ClientContextChunk[] {
  ...
  const overlapWords = Math.max(0, Math.min(options.overlapWords ?? DEFAULT_OVERLAP_WORDS, Math.floor(maxChunkWords / 4)));
...
export function summarizeChunkMetadata(chunks: ClientContextChunk[]) {
  return {
