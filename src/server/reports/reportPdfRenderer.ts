...
function drawCoverBranding(doc: PDFKit.PDFDocument, input: PdfReportRenderInput, x: number, y: number) {
  ...
  if (!branding || (!branding.companyLogo && !branding.clientLogo && !branding.companyName && !branding.clientName)) {
    doc.fillColor(THEME.muted).font("Helvetica").fontSize(9).text("Report branding: Shift Evidence", x, y, {
...
function addPageNumbers(doc: PDFKit.PDFDocument) {
  ...
  for (let i = range.start; i < range.start + range.count; i += 1) {
    ...
    doc
      ...
      .stroke();
    doc.fillColor(THEME.faint).font("Helvetica").fontSize(8).text("Powered by Shift Evidence - Evidence-based readiness assessment", MARGIN, footerTextY, {
      width: 360,
      lineBreak: false,
    });
